 ...
 */
void CProcessEnvironment::setVariable(const string &variable,
		const string &value)
{
 ...
 * effect.
 */
void CProcessEnvironment::clearVariable(const string &variable)
{
	map<string, string>::iterator iter;
	if ((iter = _envmap.find(variable)) != _envmap.end())
		_envmap.erase(iter);
 ...
 * empty string.
 */
string CProcessEnvironment::lookupValue(const string &variable) const
{
 ...
 */
char** CProcessEnvironment::toNullTerminatedArray() const
{
	...
	for (int i = 0; i < size; i++) {
		...
		CStringHelper::strlcpy(array[i], str.c_str(), str.length() + 1);
		iter++;
	}
 ...
 */
void CProcessEnvironment::freeNullTerminatedArray(char **array)
{
	// Free elements.
 ...
 */
CProcessEnvironment CProcessEnvironment::systemEnvironment()
{
